...
const createTemplate = (name: string) => {
  let content = `# ${name} Product Requirement Document\n`;
  content += `
  ---
...
export default tool({
  description: "Initializes a new PRD",
  args: {
    location: tool.schema.string().describe("The PRD directory"),
    name: tool.schema.string().describe("The name of the new PRD"),
  },
  async execute(args, context) {
    const outPath = join(args.location, `${args.name}-PRD.md`);
    if (!existsSync(args.location)) {
      mkdirSync(args.location);
    }
    try {
      const template = createTemplate(args.name);
      writeFileSync(outPath, template, "utf-8");
      return `Sucess:\nWrote to ${outPath}\n\n${template}`;
    } catch (error) {
      return `Error creating PRD '${args.name}': ${error instanceof Error ? error.message : String(error)}`;
    }
  },
});
